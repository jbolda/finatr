...
export class BarChartSpecific extends Component {
  componentDidMount() {
    let svgBar = barBuild.init(
      350,
      'bar-section',
      this.props.account.account.name.replace(/ /g, '-')
    );
    let svgLine = barBuild.init(
      350,
      'line-section',
      this.props.account.account.name.replace(/ /g, '-')
    );
    ...
    return (
      <div>
        ...
        <div
          ...
          id={this.props.account.account.name.replace(/ /g, '-')}
          style={{ overflow: 'auto' }}
        />
      </div>
    );
